...
if ! is_bash; then
    echo >&2 'The shell provides too few functions, please configure them manually.'
    return 1
fi
...
# 这个方法已经不再使用
# 之前的考虑是尽可能兼容 sh，以及没有安装 ip 命令的场景，想尽可能增加兼容性
# 但是实际使用中发现：这样做增加了很多复杂度和不确定性，而且应用场景有限
# 即便是在 Alpine 这样默认使用 sh 的系统上，也可以安装最新版本的 Bash
# 大多数系统默认就使用 Bash
# ip 也是大部分 Linux 系统默认就安装的
# 对于少数例外情况，手工做简单调整即可
# 以下方法虽然不再使用，但不要删除，留存备查
# proc_network() {
