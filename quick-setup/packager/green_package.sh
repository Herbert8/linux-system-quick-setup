...
prepare_dir() {
    ...
    # 工具包文件名
    tool_package_name=tools-pkg-${ARCH}.tar.gz
...
select_standalone_tool() {
    (
        ...
        cd "$PROJECT_ROOT/components/platform/$ARCH/standalone_tools" &&
            cp -v "${tools_list[@]}" "$TOOL_DIR"/ # coreutils \
        # curl8 \
    )
}
